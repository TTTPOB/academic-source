...
class Settings(BaseModel):
    data_dir: Path = Field(default_factory=lambda: Path(os.environ.get("ACADEMIC_SOURCE_DATA_DIR", "~/.academic-source")).expanduser())
    ...
    def load(cls, data_dir: Path | None = None) -> "Settings":
        ...
        if path.exists():
            settings = cls.model_validate({**json.loads(path.read_text(encoding="utf-8")), "data_dir": settings.data_dir})
        return settings
