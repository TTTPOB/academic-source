...
- Elsevier 一条请求路径曾对 `view=FULL` 返回 400（无效 view 参数），另一请求路径对默认 XML 与 `FULL` 均返回 200；因此默认 XML 是当前实现选择，不是对 `FULL` 普遍非法的判断。XML 200 不代表有正文权限，PDF 200 也可能只是单页摘要；离线测试只证明请求参数、附件候选与拒绝预览的代码行为。
- Science 挑战与付费墙保持区分；可选 CDP 后端通过已连接 Chrome 的文章页同源分块获取 PDF，不依赖浏览器 PDF 阅读器或下载事件。已有人工连接实验与离线模拟不等于服务生产环境的全链路验收，真实网站权限仍取决于现有会话与订阅。
...
- GHCR 既有镜像曾由 GitHub Actions 构建发布；本次 CDP 客户端镜像改动仍待 CI 构建，镜像不内置浏览器。NAS 的旧版 Python/systemd 验证与本次新 wheel/CDP 的生产验收应分开记录，不将本地 wheel 构建等同于容器运行验收。
