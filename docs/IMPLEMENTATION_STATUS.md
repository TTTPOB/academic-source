...
| 发布 | 原 PyPI 发布 workflow 改为空操作提示；插件/市场清单移出自动发现位置，归档到 docs/upstream-integrations |
...
- 旧 scansci_pdf 模块仍较大，其中有尚未移除的历史 CLI/工具实现；它们不是新项目的公开接口承诺。后续可以在来源行为验收后继续缩小这层，而不是为了目录美观先删掉难维护的站点知识。
