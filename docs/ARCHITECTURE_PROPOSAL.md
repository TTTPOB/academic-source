...
状态：讨论稿，未批准实施。本文不代表现有代码已经具备这些能力。
...
本轮只设计，不改业务代码、不安装依赖、不访问文献站点。原生 JATS 与模型转换暂不实施，仅要求核心数据模型不把论文等同于 PDF。
...
待人工看代码后决定：是否采用这套应用边界，是否从一开始引入轻量任务 API，以及旧 CLI/MCP 的兼容范围。细部命名可以调整，核心选择是“保留来源知识，统一业务和文件契约”。
